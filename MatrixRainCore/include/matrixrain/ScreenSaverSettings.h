...
{
    static constexpr float MIN_DENSITY                 = 0.0f;
    ...
    static float ClampDensity (float value);
...
{
    m_density               = ClampDensity (m_density);
...
inline float ScreenSaverSettings::ClampDensity (float value)
{
    return std::clamp (value, MIN_DENSITY, MAX_DENSITY);
}
