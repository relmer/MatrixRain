...
- Creates a `UsageDialog` with the `UsageText` reference
...
- No text grid or column activity map step — `UsageDialog` receives the `UsageText` directly and uses `IDWriteTextLayout` with proportional font for per-character positioning
