...
- [x] T004 Create the calibration and benchmark harness `tools/HdrCalibration/HdrCalibration.vcxproj` + `main.cpp`, linking `MatrixRainCore.lib`, added to `MatrixRain.sln` but built in no shipped configuration. Keep `main` thin: parse `--adapter warp|hardware`, `--mode reference|compare|benchmark`, `--baseline-dir`. **Reference mode**: hidden 1920×1080 window, `RandomSource::Reseed (1)`, fixed elapsed time, render one frame per named settings case (defaults; glow intensity min and max; glow size min and max; scanlines on at Style 1 and 100; custom color `RGB(0, 128, 255)`), read back the back buffer and write it losslessly to `baseline/<case>.png`, printing `MeanLuminance` per case (FR-006). **Compare mode**: render the same cases again, diff each against its baseline PNG with `CompareFrames`, print the difference summary, and write an amplified `baseline/<case>.diff.png` showing where the render moved. **Benchmark mode**: render 600 frames per quality preset and print mean and p95 GPU ms/frame using D3D11 timestamp queries (Constitution II)
- [x] T005 Capture the reference: on this branch after T002–T004 and **before any pipeline change** (rendering is still identical to v1.6, since only the RNG source changed), run the harness in reference mode on WARP and in benchmark mode on WARP and the hardware adapter. Record all numbers in `baseline.md` with the commit hash. Verify the capture by immediately re-running compare mode: every case must report zero difference
...
- [ ] T018 [US1] Calibrate with the harness (T004): run **compare mode** on the new pipeline for **every** settings case against the T005 baseline. Tune the extract thresholds (T014), `kBloomCeiling` (T015) and, if needed, the glow-intensity mapping until each case meets the thresholds in `baseline.md` (FR-005, FR-006, SC-001). The difference will not be zero — Phase 1 deliberately removes banding and fixes halo blending — so **inspect each `.diff.png`** and confirm the remaining difference is the intended improvement and not a drift in exposure, glow size or hue. Record the final constants, per-case numbers and that judgement in `baseline.md`, with a comment in `RenderSystem.cpp` citing the calibration
