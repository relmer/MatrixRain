...
- [x] T020 [US1] **Done, gate not fully met; see `baseline.md`, "Performance gate result".** Same-session comparison against the T005 build: six of nine hardware configurations inside 5%, Medium on both monitors and High on the landscape monitor over by 0.2 to 2.1 points (14 to 19 µs per frame); WARP well over. Found and recovered: FP16 targets (bandwidth; now R11G11B10), glyph blending of zero-coverage pixels (now discarded, bit-identical), a redundant decode+encode in the composite. The remainder is the encode work the v1.6 match needs. Presets were not retuned; that changes their look and is Rob's call. Performance gate (Constitution II, SC-006): re-run the harness benchmark mode (WARP and hardware) and the T001 manual FPS readings. No preset may be more than 5% slower than T005/T001. If one is, retune that preset in `MatrixRainCore/QualityPresets.cpp` and record why
