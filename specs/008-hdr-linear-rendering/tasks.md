...
- [x] T004 Create the calibration and benchmark harness `tools/HdrCalibration/HdrCalibration.vcxproj` + `main.cpp`, linking `MatrixRainCore.lib`, added to `MatrixRain.sln` but built in no shipped configuration. Keep `main` thin: parse `--adapter warp|hardware`, `--mode reference|compare|benchmark`, `--baseline-dir`. **Reference mode**: hidden 1920×1080 window, `RandomSource::Reseed (1)`, fixed elapsed time, render one frame per named settings case (defaults; glow intensity min and max; glow size min and max; scanlines on at Style 1 and 100; custom color `RGB(0, 128, 255)`), read back the back buffer and write it losslessly to `baseline/<case>-dpi<percent>.png`, printing `MeanLuminance` per case (FR-006). **Every case is captured at display scale 100%, 125% and 150%** (24, 30 and 36 px cells): scanline pitch is derived from the cell, so a single scale would pin the scanline cases to one side of the 3 px pitch floor and leave the other unguarded. **Compare mode**: render the same cases again at the same scales, diff each against its baseline PNG with `CompareFrames`, print the difference summary, and write an amplified `.diff.png` showing where the render moved. **Benchmark mode**: render 600 frames per quality preset and print mean and p95 GPU ms/frame using D3D11 timestamp queries (Constitution II)
