...
- [ ] T044 [US3] Tune and validate on hardware: confirm the default highlight brightness of 80 is comfortable in a dim room (if not, adjust `DEFAULT_HIGHLIGHT_BRIGHTNESS` and T036's expectation together), and tune `kHighlightGlowStrength` (research R14) so a head's glow above white reads as the same glow, brighter, not as a separate halo. Run [quickstart.md](quickstart.md) Phase 3 checks 1–9. Re-run the benchmark and manual FPS readings with HDR mode Auto on an HDR monitor against the 5% threshold (SC-006). Record the results
