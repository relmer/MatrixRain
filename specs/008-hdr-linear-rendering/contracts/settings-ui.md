...
| Highlight brightness | `IDC_HIGHLIGHT_SLIDER` + `_VALUE` + `_PROMPT` | Trackbar 0–100, value shown as `NN%` | Live; greyed when HDR mode is Off |
| Info | `IDC_HDR_INFO` | ⓘ owner-draw button | Tooltip: "Affects monitors with Windows HDR turned on. Other monitors are unchanged." |

- The group is **greyed** while no monitor is currently in HDR output mode,
...
- Greying follows the sheet-scoped pattern from the v1.6 scanline fix: the
