...
- **FR-001**: The system MUST composite glyphs and overlays into the scene on values proportional to emitted light (linear light), in float render targets, and MUST hold every intermediate image in float so nothing is quantized to 8 bits before the final encode. The *appearance* of every effect a user can tune MUST reproduce v1.6's on-screen result: the trail fade curve, the weight of antialiased glyph edges, the darkening the Scanlines Intensity slider applies, and the glow in full — its strength, falloff, hue, its behavior where streaks overlap, and the way it lands on glyph bodies. The glow chain (extract, blur, saturation, composite) therefore runs on encoded values exactly as v1.6's did, and the result is converted to linear light for output. (Revised twice during Phase 1 calibration. The original wording put the glow chain and scanline darkening in linear light; each produced a visible difference from v1.6 that was matched piece by piece until the overlap behavior, which Rob judged worse for legibility, made it clear the glow should simply be v1.6's.) Principle: light combines in linear light; everything a user can tune keeps its v1.6 meaning. (Revised during Phase 1 calibration: the original wording put scanline darkening in linear light, which made every saved Intensity value about 50% weaker than the user had set it.)
