...
- **FR-001**: The system MUST perform light *combination* — compositing glyphs and overlays into the scene, and the blur chain in which neighboring halos overlap and sum — on values proportional to emitted light (linear light), not on gamma-encoded values. The *appearance* of each effect a user can tune MUST reproduce v1.6's on-screen result: the trail fade curve, the weight of antialiased glyph edges, the darkening the Scanlines Intensity slider applies, and the glow's strength, falloff shape, hue and the way it lands on glyph bodies. Those terms are computed in gamma space, where v1.6 authored them, and converted to linear light afterward. Principle: light combines in linear light; everything a user can tune keeps its v1.6 meaning. (Revised during Phase 1 calibration: the original wording put scanline darkening in linear light, which made every saved Intensity value about 50% weaker than the user had set it.)
