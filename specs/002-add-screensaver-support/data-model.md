...
|-------|------|-------------|------------------------|
| densityPercent | `int` | Character density percentage relative to base configuration | Clamped to `[0, 100]` (0 renders exactly one streak head), default `50`. Max streaks = `(viewportWidth / charWidth) * 2.4`. Stored in registry as `REG_DWORD` and used directly by `DensityController`. |
