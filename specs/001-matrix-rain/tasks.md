...
- [X] T179 [P] Create FPSCounter class in MatrixRainCore/include/matrixrain/FPSCounter.h with frame time tracking and averaging
- [X] T180 Implement FPSCounter::Update in MatrixRainCore/src/ui/FPSCounter.cpp to calculate rolling average FPS over 1-second window
- [X] T181 Implement FPSCounter::GetFPS in MatrixRainCore/src/ui/FPSCounter.cpp to return current FPS value
- [X] T182 [P] Create ColorScheme enum in MatrixRainCore/include/matrixrain/ColorScheme.h with Green, Blue, Red, Amber values
- [X] T183 [P] Add GetColorRGB helper function in MatrixRainCore/src/state/ColorScheme.cpp returning RGB values for each scheme
- [X] T184 Add ColorScheme member to ApplicationState in MatrixRainCore/include/matrixrain/ApplicationState.h with default Green
- [X] T185 Implement ApplicationState::CycleColorScheme in MatrixRainCore/src/state/ApplicationState.cpp to advance through color sequence
- [X] T186 Add VK_C key handler in InputSystem::ProcessKeyDown to call ApplicationState::CycleColorScheme in MatrixRainCore/src/input/InputSystem.cpp
- [X] T187 Wire FPSCounter to Application class in MatrixRainCore/src/Application.cpp, update per frame
- [X] T188 Implement FPS text rendering in RenderSystem::Render in MatrixRainCore/src/rendering/RenderSystem.cpp at bottom-right corner using Direct2D/DirectWrite
- [X] T189 Update CharacterInstance color calculation to use current ColorScheme from ApplicationState in MatrixRainCore/src/animation/CharacterInstance.cpp
- [X] T190 Update pixel shader to use dynamic color from constant buffer instead of hardcoded green in MatrixRainCore/src/rendering/RenderSystem.cpp
- [X] T191 Verify integration test for color cycling in MatrixRainTests/integration/ColorCycleTests.cpp
- [X] T192 Verify FPS counter displays and updates correctly in MatrixRainTests/integration/FPSDisplayTests.cpp
