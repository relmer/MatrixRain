...
- [ ] T043 [US2] `MatrixRain\ConfigDialog.cpp`: register comctl32 `TOOLTIPS_CLASS` tooltip on each disabled glow control with the correct per-tab text — DEFERRED (greyed-out controls already convey disabled state; tooltips on WS_DISABLED controls require a transparent parent-relay tooltip per page, tracked as follow-up)
...
- [ ] T051 [US3] `MatrixRainCore\RenderSystem.{h,cpp}`: scanline post-pass + m_postBloomTarget — DEFERRED (render-pipeline integration; settings persist and the .hlsl is in place; no visual effect until this lands)
- [ ] T052 [US3] `MatrixRainCore\RenderSystem.cpp` per-frame upload — DEFERRED (paired with T051)
