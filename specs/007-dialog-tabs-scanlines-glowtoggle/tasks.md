...
- [ ] T001 Reserve new control / dialog / string IDs in `MatrixRain\resource.h`: `IDD_VISUALS_PAGE`, `IDD_PERFORMANCE_PAGE`, `IDS_VISUALS_TAB_TITLE`, `IDS_PERFORMANCE_TAB_TITLE_INITIAL`, `IDS_PERFTAB_TITLE_FORMAT`, `IDT_PERF_TITLE_TIMER`, `IDC_GLOW_ENABLED_CHECK`, `IDC_SCANLINES_ENABLED_CHECK`, `IDC_SCANLINES_INTENSITY_SLIDER` + `_LABEL` + `_VALUE` + `_INFO`, `IDC_SCANLINES_STYLE_SLIDER` + `_LABEL` + `_VALUE` + `_INFO`, `IDC_COLOR_COMBO` (if not already), placeholders for the per-tab control IDs the .rc split will need (FR-001, FR-005, FR-008, FR-009, contracts/propertysheet.md)
- [ ] T002 [P] Add `<commdlg.h>` and `<prsht.h>` to `MatrixRain\pch.h` (Windows headers section, alphabetised, preserving the 3-blank-line group separator); add `<atomic>` to `MatrixRainCore\pch.h` if not already present (research.md R1, R3, R5)
- [ ] T003 [P] Create directory `MatrixRainCore\Shaders\` and add a `.gitkeep` or initial placeholder so the scanline shader has its eventual home; add the directory to `MatrixRainCore.vcxproj` as a None/Content item if needed (plan.md "Project Structure", FR-022)
- [ ] T004 [P] Create skeleton `MatrixRainCore\ScanlineStyleMapping.h` declaring `float ScanlineLineCount(int style) noexcept;` (header-only or paired .cpp — match project convention for tiny pure functions) (FR-023, data-model.md §7)
- [ ] T005 [P] Create skeleton `MatrixRainCore\ScanlineStyleMapping.cpp` with `#include "pch.h"` + own header + body returning `0.0f` (intentional Red so T006's tests fail first) (FR-023)
- [ ] T006 [P] Write `MatrixRainTests\unit\ScanlineStyleMappingTests.cpp` asserting line count at style ∈ {1, 25, 50, 75, 100} matches {~981, ~622, ~387, ~241, ~150} within ±2 lines, plus clamp-invariance for inputs already at the [1,100] endpoints (Red against T005 stub) (FR-023, data-model.md §7)
- [ ] T007 Implement real body of `MatrixRainCore\ScanlineStyleMapping.cpp` — `return 1000.0f * std::pow(0.15f, static_cast<float>(style) / 100.0f);` — making T006 Green; commit pair as one logical change (FR-023)
...
## Phase 2: User Story 4 — Fade-Timer Feature Removal (Priority: P1)
...
- [ ] T055 [US3] Migration sanity: with no `Scanlines*` registry values present, launching `MatrixRain.exe /c` shows scanlines ON in preview on first frame (matches SC-011 / quickstart §US6 step 2) (FR-028, FR-028d, SC-011)
...
`ChooseColorW`, support same-item re-click force-reopen via `CBN_SELENDOK`
per research.md R4, persist the 16-swatch palette unconditionally as
REG_BINARY per R5, route the active custom RGB through `SharedState` to
the render thread.
...
- [ ] T056 [P] [US5] Create `MatrixRainTests\unit\ColorSchemeTests.cpp` (or extend existing): assert `ColorScheme::Custom == 4`; assert existing ordinals Green=0, Amber=1, White=2, Cycle=3 unchanged; round-trip through registry; assert `RenderSystem`-side selection logic picks `customColor` when scheme == Custom (via a small pure helper if RenderSystem isn't directly testable) (Red) (FR-033, FR-034, data-model.md §2)
...
- [ ] T059 [P] [US5] `MatrixRainCore\ColorScheme.{h,cpp}`: append `Custom = 4` to `enum class ColorScheme`; update any to-string / from-string helpers to handle the new variant (Green for T056) (FR-033, data-model.md §2)
...
- [ ] T063 [US5] `MatrixRain\MatrixRain.rc`: add `L"Custom…"` as the fifth entry in the `IDC_COLOR_COMBO` on `IDD_VISUALS_PAGE` (ordinal 4, after Green/Amber/White/Cycle) (FR-006)
- [ ] T064 [US5] `MatrixRain\ConfigDialog.cpp`: implement combo handler tracking `s_lastColorComboIndex` (page-state struct member; init from loaded scheme). Handle `CBN_SELCHANGE`: if new index is Custom → open chooser; else update scheme normally. Handle `CBN_SELENDOK`: if committed index is Custom AND `s_lastColorComboIndex` was already Custom → same-item re-click → force-open chooser. Always update `s_lastColorComboIndex` after a handled event (FR-029, research.md R4)
