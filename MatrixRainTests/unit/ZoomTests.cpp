...
// Manually modify the streak position for testing
        ...
        TEST_METHOD (TestZoomDoesNotAffectXY)
        {
            ...
            if (!streaks.empty ())
            {
                ...
                // Apply zoom using smaller time steps to avoid assertion failures
                for (int i = 0; i < 10; ++i)
                {
        ...
        {
            // Test zoom over extended runtime with large time steps
            // Validates wrapping behavior and numerical stability
            ...
            if (!streaks.empty ())
            {
                ...
                // Simulate 10 complete zoom cycles using large time steps
                // At 5.0 units/sec, each cycle through 100 units takes 20 seconds
                // 10 cycles = 200 seconds total
                // Use 10-second time steps = 20 updates instead of 10,000
                for (int i = 0; i < 20; ++i)
                {
